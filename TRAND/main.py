...
def require_args():
    ...
    cfg.add_argument('--max-epoch', default=20, type=int,
                        help='max epoch per round. (default: 200)')
    cfg.add_argument('--max-round', default=2, type=int,
...
def main():
    ...
    trainloader = tordata.DataLoader(dataset=trainset, batch_size=32,
                                     collate_fn=train_collate_fn, num_workers=num_workers)
    testloader = tordata.DataLoader(dataset=testset, batch_size=16,
    ...
    #data parallel
    if device == 'cuda':
        print(range(len(cfg.gpus.split(','))))
    ...
    if cfg.load_pretrained:
        load(net, optimizer, WORK_PATH, model_name, save_name, restore_iter=1000)
